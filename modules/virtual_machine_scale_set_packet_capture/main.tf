resource "azurerm_virtual_machine_scale_set_packet_capture" "this" {
  ...
  dynamic "machine_scope" {
    for_each = var.machine_scope_include_instance_ids != null || var.machine_scope_exclude_instance_ids != null ? [1] : []
    content {
      include_instance_ids = var.machine_scope_include_instance_ids
      exclude_instance_ids = var.machine_scope_exclude_instance_ids
    }
  }
}
